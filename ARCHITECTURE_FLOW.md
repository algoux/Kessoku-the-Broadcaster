...
- **主进程**: 专门处理 WebSocket 信令和系统级操作
- **渲染进程**: 专门处理 UI 交互和 WebRTC 媒体

### 2. **连接稳定性**

- WebSocket 连接在主进程中维护，避免页面刷新影响
- 连接断开自动重连机制
...
- 本地预览与推流分离，停止推流不影响本地录制
- 自动设备检测和流恢复机制
- 实时状态同步和错误处理

## 🔍 关键技术点

1. **IPC 通信**: Electron 主进程与渲染进程的安全通信
2. **WebSocket 信令**: 实时的推流控制和状态同步
3. **WebRTC 传输**: 高质量的实时音视频传输
4. **设备管理**: 摄像头、麦克风、屏幕共享的统一管理
5. **状态管理**: 连接状态、推流状态、设备状态的实时同步
